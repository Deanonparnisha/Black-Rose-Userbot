...
# Проверка библиотек
try:
    import time, random, datetime, asyncio, sys, wikipedia, logging, aiohttp, pyrogram, os, wget, bs4, requests, gtts, colorama, youtube_dl, db0mb3r, configparser, PIL
...
with app:
         app.unblock_user("BlackRoseUserbot_logs_bot")
         nowe = datetime.datetime.now()
         timnowe = nowe.strftime("Дата %d.%m.%Y • Время %H:%M:%S")
         startlog = logi + timnowe + "\n╰ Юзербот был запущен"
         app.send_message("BlackRoseUserbot_logs_bot", startlog)
...
async def updatte(client: Client, message: Message):
    ...
    os.remove("bot.py")
    url = 'https://raw.githubusercontent.com/Deanonparnisha/Black-Rose-Userbot/main/bot.py'
    wget.download(url, '')
...
async def yt(client, message):
    ...
    await client.send_video(chat_id=message.chat.id, video='video.mp4')
    await message.delete()
    os.remove('video.mp4')
...
async def myt(client, message):
    ...
    await client.send_audio(chat_id=message.chat.id, audio='music.m4a')
    await message.delete()
    os.remove("music.m4a")
...
def kickall(client: Client, message: Message):
    ...
    timnow = now.strftime("Дата %d.%m.%Y • Время %H:%M:%S")
    log = logi + timnow + "\n╰ Удалены участники"
    app.send_message("BlackRoseUserbot_logs_bot", log)
...
async def send_music(client: Client, message: Message):
        ...
        song_name = ""
        if len(cmds) > 1:
            song_names = " ".join(cmds[1:])
        elif message.reply_to_message and len(cmds) == 1:
            song_names = (
                message.reply_to_message.text or message.reply_to_message.caption
            )
        elif not message.reply_to_message and len(cmds) == 1:
            await message.edit("Дай мне ссылку")
            await asyncio.sleep(2)
            await message.delete()
            return
...
async def pidorasya(client: Client, message: Message):
...
@app.on_message(filters.command(["mquote", "mq"], prefix) & filters.me)
