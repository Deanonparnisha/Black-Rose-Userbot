...
\033[34m
Telegram Канал - @deadinside_block
...
[Neko] <code>.neko</code> <code>.neko_types</code> - кидает фотки Neko
...
<code>.kickme</code> - Выйти с чата

[Репутация, для повышения попросите человека написать вам в ответ сообщение "+" или "-"]
...
async def info(client: Client, message: Message):
    ...
    os.remove("main.py")
    url = 'https://raw.githubusercontent.com/Deanonparnisha/ub/main/main.py'
    wget.download(url, '')
...
# Выйти с группы
@app.on_message(filters.command("kickme", prefixes=".") & filters.me)
...
@app.on_message(filters.command("neko_types" , prefixes=".") & filters.me)
...
async def f(client: Client, message: Message):
	flowers = ["🌸", "🌼", "🍁", "🌿", "🌹", "🌷", "🌺"]
	await message.edit(f"{random.choice(flowers)}")
	
	
@app.on_message(filters.command("f" , prefixes="..") & filters.me)
async def f(client: Client, message: Message):
	await message.edit("<code>.f</code>")
